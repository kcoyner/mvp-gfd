...
export class MessageService {
  ...
  addMessage(message: Message) {
    ...
    const token = localStorage.getItem('token')
      ...
      : '';
    return this.http.post('http://localhost:3333/message' + token, body, {headers: headers})
  ...
  updateMessage(message: Message) {
      ...
      const token = localStorage.getItem('token')
        ? '?token=' + localStorage.getItem('token')
        : '';
      return this.http.patch('http://localhost:3333/message/' + message.messageId + token, body, {headers: headers})
  ...
  deleteMessage(message: Message) {
    ...
    const token = localStorage.getItem('token')
      ? '?token=' + localStorage.getItem('token')
      : '';
    return this.http.delete('http://localhost:3333/message/' + message.messageId + token)
      .map((response: Response) => response.json())
      .catch((error: Response) => Observable.throw(error.json()));
      // this returns an observable that can be subscribed to
  }
